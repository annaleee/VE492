...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for distribution in livingGhostPositionDistributions:
            most_likely_position.append(distribution.argMax())
        ghostDistance = []
        for ghostPos in most_likely_position:
            ghostDistance.append(self.distancer.getDistance(pacmanPosition, ghostPos))
        closest_ghost = most_likely_position[ghostDistance.index(min(ghostDistance))]
        successor_distance = []
        for action in gameState.getLegalPacmanActions():
            successor_distance.append(
                self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closest_ghost))
        return legal[successor_distance.index(min(successor_distance))]
