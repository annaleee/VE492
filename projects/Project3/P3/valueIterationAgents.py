...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for i in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                if self.mdp.isTerminal(state):
                    continue
                values[state] = max(self.getQValue(state, action) for action in
    ...
    def computeActionFromValues(self, state):
        ...
        return self.mdp.getPossibleActions(state)[Q_values.index(max(Q_values))]
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            if self.mdp.isTerminal(state):
                continue
            self.values[state] = max(self.getQValue(state, action) for action in
                                     self.mdp.getPossibleActions(state))
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for s in states:
            ...
            for action in self.mdp.getPossibleActions(s):
                for nextState, probability in self.mdp.getTransitionStatesAndProbs(s,action):
                    if nextState not in prodecessors.keys():
                        prodecessors[nextState]=set()
                    if probability:
                        prodecessors[nextState].add(s)
        ...
        for s in states:
            ...
            PriorityQueue.push(s,-abs(self.values[s]-max(Q_values)))
        for i in range(self.iterations):
            ...
            for p in prodecessors[state]:
                if self.mdp.isTerminal(p):
                    continue
                diff=abs(max(self.getQValue(p, action) for action in self.mdp.getPossibleActions(p))-self.values[p])
                if diff>self.theta:
